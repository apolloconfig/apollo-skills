#!/usr/bin/env python3
"""Build Apollo release notes and announcement drafts."""
...
def _shorten_usage_line(line: str, limit: int = 170) -> str:
    lowered = line.lower()
    users_idx = lowered.find("users can ")
    if users_idx >= 0:
        return _truncate_text(line[users_idx:].strip(), limit=limit)
    curl_idx = lowered.find("curl ")
    if curl_idx >= 0:
        return _truncate_text(line[curl_idx:].strip(), limit=limit)
    you_can_idx = lowered.find("you can ")
    if you_can_idx >= 0:
        return _truncate_text(line[you_can_idx:].strip(), limit=limit)

    segments = re.split(r"(?<=[,\.\!\?;:\u3002\uff0c\uff1b])\s+", line)
    for segment in segments:
        candidate = segment.strip().rstrip(",\uFF0C")
        if 24 <= len(candidate) <= limit:
            return candidate
    return _truncate_text(line, limit=limit)
...
def _split_candidate_lines(markdown_text: str) -> list[str]:
    ...
    for raw_line in markdown_text.splitlines():
        ...
        if line.startswith("<"):
            continue
        if re.search(r"https?://github\.com/user-attachments/", line):
            continue
        if len(line) < 24:
            continue
        lines.append(line)
...
def _score_usage_line(line: str) -> int:
    ...
    if "users can" in lowered or "you can" in lowered:
        score += 3
    if "without" in lowered and ("proxy" in lowered or "manual" in lowered):
        score += 2
    if "curl " in lowered:
        score += 4
    ...
    if "/issues/" in lowered:
        score -= 3
    if lowered.startswith("see details at"):
        score -= 3
    if "newbuilder(" in lowered:
        score -= 3
    if "summary by coderabbit" in lowered or "walkthrough" in lowered:
        score -= 5
    if "follow this checklist" in lowered:
        score -= 4
    if "auto-generated comment" in lowered:
        score -= 5
    if lowered.startswith("fixes #"):
        score -= 4
    if re.search(r"[A-Za-z_][A-Za-z0-9_]*\s*=\s*[A-Za-z_][A-Za-z0-9_\.]*\(", line):
        score -= 2
    return score
...
def _pick_best_usage_line(lines: list[str]) -> Optional[str]:
    ...
    if best_score <= 0:
        return None
    return _shorten_usage_line(best_line)


def _summarize_doc_files(files: list[str]) -> Optional[str]:
    doc_files = [path for path in files if path.startswith("docs/")]
    if not doc_files:
        return None
    doc_names: list[str] = []
    for path in doc_files:
        ...
        if len(doc_names) >= 2:
            break
    top_paths = ", ".join(f"`{name}`" for name in doc_names)
    return _truncate_text(f"Detailed usage notes are documented in {top_paths}.", limit=160)
...
def _fetch_doc_patch_lines(repo: str, pr_number: int) -> list[str]:
    ...
    for item in payload:
        ...
        if not isinstance(filename, str) or not filename.startswith("docs/"):
            continue
...
def _parse_release_changes_block(
    ...
    header = f"Apollo {release_version}"
    start_index = -1
...
def format_change_lines(entries: Iterable[ChangeEntry], repo: Optional[str] = None) -> list[str]:
    ...
    for entry in entries:
        ...
        if repo and entry.pr_number and pr_url:
            meta = _fetch_pr_metadata(repo, entry.pr_number)
            author_login: Optional[str] = None
            if meta:
                author_login = meta.author_login
            if author_login:
                lines.append(f"* {summary} by @{author_login} in {pr_url}")
            else:
...
def _score_highlight_candidate(entry: ChangeEntry) -> int:
    original = (entry.summary or entry.raw_text).strip().lower()
    text = _clean_summary_for_highlight(entry.summary or entry.raw_text).lower()
    if not text:
        return -1
    ...
    if re.match(r"^(test|chore|ci|docs|build)(\([^)]+\))?:", original):
        score -= 3
    if any(token in text for token in ["feature", "support", "add ", "added ", "enhance", "new "]):
        score += 2
    if any(token in text for token in ["security", "auth", "permission"]):
        score += 1
...
def _normalize_usage_hint(text: str) -> str:
    ...
    normalized = re.sub(r"^in some application scenarios,?\s*", "", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"^after updating the project,?\s*", "", normalized, flags=re.IGNORECASE)
    normalized = re.sub(
        r"^after starting the service and making some requests to pull configurations,?\s*",
        "",
        normalized,
        flags=re.IGNORECASE,
    )
    normalized = re.sub(r"like this way[:\uff1a]?\s*$", "", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\s*See details at https?://\S+\s*$", "", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\s*See details:?\s*https?://\S+\s*$", "", normalized, flags=re.IGNORECASE)
    ...
    if lowered.startswith("we can "):
        normalized = f"Users can {normalized[7:].strip()}"
        lowered = normalized.lower()
    if lowered.startswith("users can "):
        normalized = f"Users can {normalized[10:].strip()}"
        lowered = normalized.lower()
    if lowered.startswith("you can ") or lowered.startswith("users can "):
...
def _build_summary_sentence(summary: str) -> str:
    ...
    if lower.startswith("support "):
        return _ensure_sentence_end(f"Adds support for {summary[8:].strip()}")
    if lower.startswith("provide "):
        return _ensure_sentence_end(f"Provides {summary[8:].strip()}")
    if lower.startswith("add "):
        return _ensure_sentence_end(f"Adds {summary[4:].strip()}")
    if lower.startswith("added "):
        return _ensure_sentence_end(f"Adds {summary[6:].strip()}")
    if lower.startswith("fix "):
        return _ensure_sentence_end(f"Fixes {summary[4:].strip()}")
    if lower.startswith("enhance "):
        return _ensure_sentence_end(f"Enhances {summary[8:].strip()}")
    if lower.startswith("enhanced "):
        return _ensure_sentence_end(f"Enhances {summary[9:].strip()}")
    return _ensure_sentence_end(f"Includes {summary}")
...
def _build_highlight_body(
    ...
    if not summary:
        return f"Apollo {release_version} is now available."
...
def build_highlights(
    ...
    if missing_prs:
        ...
        raise ValueError(
            f"Selected highlight PRs not found in CHANGES.md for Apollo {release_version}: {missing_str}"
        )
...
def version_to_sql_number(version: str) -> int:
    major, minor, patch = parse_semver(version)
    return major * 100 + minor * 10 + patch


def detect_upgrade_info(
    ...
    previous_version = strip_v_prefix(previous_tag)
    from_code = version_to_sql_number(previous_version)
    to_code = version_to_sql_number(release_version)
    folder = f"v{from_code}-v{to_code}"
    folder_path = delta_src_root / folder

    config_name = f"apolloconfigdb-v{from_code}-v{to_code}.sql"
    portal_name = f"apolloportaldb-v{from_code}-v{to_code}.sql"
    config_file = config_name if (folder_path / config_name).exists() else None
    portal_file = portal_name if (folder_path / portal_name).exists() else None

    return UpgradeInfo(
        has_schema_change=bool(config_file or portal_file),
        folder=folder,
...
def _build_upgrade_sql_link(
    repo: str,
    release_version: str,
    profiles_delta_root: Path,
    folder: str,
    filename: str,
) -> str:
    relative = (profiles_delta_root / folder / filename).as_posix()
    tag = normalize_tag(release_version)
    return f"https://github.com/{repo}/blob/{tag}/{relative}"


def build_upgrade_section(
    repo: str,
    release_version: str,
    previous_tag: Optional[str],
    delta_src_root: Path,
    ...
    if previous_tag:
        previous_version = strip_v_prefix(previous_tag)
        heading = f"## How to upgrade from v{previous_version} to v{release_version}"
    else:
        previous_version = None
        heading = f"## How to upgrade to v{release_version}"

    upgrade_info = detect_upgrade_info(previous_tag, release_version, delta_src_root)

    lines = [heading]
    if upgrade_info.has_schema_change and previous_version:
        steps: list[str] = []
        if upgrade_info.config_file and upgrade_info.folder:
            config_url = _build_upgrade_sql_link(
                repo,
                release_version,
                profiles_delta_root,
                upgrade_info.folder,
                upgrade_info.config_file,
            )
            steps.append(
                f"Apply [{upgrade_info.config_file}]({config_url}) to ApolloConfigDB"
            )
        if upgrade_info.portal_file and upgrade_info.folder:
            portal_url = _build_upgrade_sql_link(
                repo,
                release_version,
                profiles_delta_root,
                upgrade_info.folder,
                upgrade_info.portal_file,
            )
            steps.append(
                f"Apply [{upgrade_info.portal_file}]({portal_url}) to ApolloPortalDB"
            )

        for index, step in enumerate(steps, start=1):
            lines.append(f"{index}. {step}")

        deploy_index = len(steps) + 1
        lines.append(
            f"{deploy_index}. Deploy v{release_version} executables with the following sequences:"
        )
        lines.append("    1. apollo-configservice")
        lines.append("    2. apollo-adminservice")
        lines.append("    3. apollo-portal")
        return "\n".join(lines)

    if previous_version:
        lines.append(
            f"There is no schema change between v{previous_version} and v{release_version}."
        )
    else:
        lines.append(f"There is no schema change for v{release_version}.")
    lines.append(
        f"So simply deploy v{release_version} executables with the following sequences:"
    )
    lines.append("1. apollo-configservice")
    lines.append("2. apollo-adminservice")
    lines.append("3. apollo-portal")
...
def build_release_markdown(
    ...
    if highlight_list:
        ...
        lines.extend(["### Release Update", f"Apollo {release_version} is now available.", ""])
    ...
    if change_list:
        ...
        lines.append(f"* Apollo {release_version} is now available.")
...
def build_announcement_markdown(
    ...
    change_list = list(change_lines)
    if change_list:
        lines.extend([line.replace("* ", "- ", 1) for line in change_list])
    else:
        lines.append(f"- Apollo {release_version} is now available.")
...
def build_release_content(
    ...
    delta_src_root: Path = Path("scripts/sql/src/delta"),
    profiles_delta_root: Path = Path("scripts/sql/profiles/mysql-default/delta"),
) -> dict[str, object]:
    if not highlight_pr_numbers:
    ...
    upgrade_section = build_upgrade_section(
        repo=repo,
        release_version=release_version,
        previous_tag=resolved_previous_tag,
        delta_src_root=delta_src_root,
        profiles_delta_root=profiles_delta_root,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Apollo release notes from CHANGES.md")
    parser.add_argument("--repo", default="apolloconfig/apollo", help="GitHub repository")
    parser.add_argument("--release-version", required=True, help="Release version, e.g. 2.5.0")
    parser.add_argument(
        "--highlight-prs",
        required=True,
        help="Comma-separated PR numbers used to compose Highlights, e.g. 5336,5361,5365",
    )
    ...
    parser.add_argument("--target-commitish", default="master", help="Release target branch/commit")
    parser.add_argument("--previous-tag", default=None, help="Optional previous tag override")
    parser.add_argument(
        "--delta-src-root",
        default="scripts/sql/src/delta",
        help="Path to SQL delta source directory",
    )
    parser.add_argument(
        "--profiles-delta-root",
        default="scripts/sql/profiles/mysql-default/delta",
        help="Path used to generate SQL links in release notes",
    )
...
def main() -> int:
    ...
    content = build_release_content(
        ...
        delta_src_root=Path(args.delta_src_root),
        profiles_delta_root=Path(args.profiles_delta_root),
    )
