#!/usr/bin/env python3
"""Apollo Java formal release orchestrator with checkpoint gating and resume support."""
...
class ReleaseFlow:
    ...
    def _preflight(self) -> None:
        ...
        required_tools = ["gh", "git", "jq", "python3", "mvn"]
        missing = [tool for tool in required_tools if shutil.which(tool) is None]
        if missing:
            raise ReleaseFlowError(f"Missing required tools: {', '.join(missing)}")

        if self.repo_root.name != "apollo-java":
            raise ReleaseFlowError(
                "Current directory must be the apollo-java repository root (directory name 'apollo-java')."
            )
        pom_path = self.repo_root / "pom.xml"
        if not pom_path.exists():
            raise ReleaseFlowError("Current directory does not contain pom.xml")
        if self._read_root_artifact_id(pom_path) != "apollo-java":
            raise ReleaseFlowError("Root pom.xml artifactId is not 'apollo-java'")
        ...
        upstream_candidates = [
            name for name, slug in remotes.items() if slug == UPSTREAM_REPO
        ]
        if not upstream_candidates:
            raise ReleaseFlowError(
                "No git remote points to github.com/apolloconfig/apollo-java"
            )
    ...
    def _prepare_release_pr(self) -> None:
        if not self._step_done("release_pr_prepared"):
            ...
            self._run_command(["git", "fetch", upstream_remote, "main"], mutate=True, check=True)
            self._run_command(
                ["git", "checkout", "-B", release_branch, f"{upstream_remote}/main"],
                mutate=True,
                check=True,
            )
            ...
            body_path.write_text(
                self._render_release_pr_body(self.args.release_version), encoding="utf-8"
            )
        ...
        create_pr = self._run_command(
            [
                ...
                head_ref,
                "--title",
                f"Release Apollo Java {self.args.release_version}",
                "--body-file",
    ...
    def _wait_release_pr_merge(self) -> None:
        ...
        while True:
            ...
            if payload.get("state") == "CLOSED":
                raise ReleaseFlowError(
                    f"Release PR #{pr_number} was closed without merge"
                )
    ...
    def _create_prerelease(self) -> None:
        ...
        self.state["announcement_body_path"] = str(
            self.repo_root / ".git" / f"announcement-{self.args.release_version}.md"
        )
        Path(self.state["announcement_body_path"]).write_text(
        ...
        cmd = [
            ...
            "main",
            "--title",
            f"{self.args.release_version} Release",
            "--notes-file",
    ...
    def _trigger_release_workflow(self) -> None:
        ...
        self._checkpoint(
            "TRIGGER_RELEASE_WORKFLOW",
            "Trigger release.yml to publish through Sonatype Central plugin.",
        )
        ...
        if self.args.dry_run:
            self._mark_step_done(
                "release_workflow_completed",
                {
                    ...
                    "release_workflow_url": "https://example.invalid/release-workflow",
                },
            )
            return
        ...
        run = self._wait_for_new_run("release.yml", started_at)
        ...
        self._mark_step_done(
            "release_workflow_completed",
            {
                ...
                "release_workflow_url": run.get("url"),
            },
        )
        self._mark_timestamp("release_workflow_completed_at")
    ...
    def _render_announcement_from_release_notes(
        cls, release_version: str, release_notes_markdown: str
    ) -> str:
        change_lines = cls._extract_section_bullets(release_notes_markdown, "What's Changed")
        contributor_lines = cls._extract_section_bullets(release_notes_markdown, "New Contributors")
        ...
        if contributor_lines:
            ...
            lines.extend(contributor_lines)
    ...
    def _create_announcement_discussion(self) -> None:
        ...
        try:
            ...
            metadata = {
                ...
                "announcement_manual_url": "https://github.com/apolloconfig/apollo-java/discussions/new?category=announcements",
            }
    ...
    def _prepare_post_release_pr(self) -> None:
        ...
        self._run_command(["git", "fetch", upstream_remote, "main"], mutate=True, check=True)
        self._run_command(
            ["git", "checkout", "-B", branch_name, f"{upstream_remote}/main"],
            mutate=True,
            check=True,
        )

        # Archive CHANGES.md before resetting it.
        ...
        if not self.args.dry_run:
            self._run_command(["git", "add", "pom.xml", "CHANGES.md", str(archived_path)], mutate=True, check=True)
            self._run_command(
        ...
        create_pr = self._run_command(
            [
                ...
                head_ref,
                "--title",
                f"Prepare Apollo Java {self.args.next_snapshot}",
                "--body-file",
                str(body_path),
            ],
            mutate=True,
    ...
    def _ensure_next_milestone(self, next_release: str) -> int:
        ...
        milestones = self._gh_api_json(
            "repos/apolloconfig/apollo-java/milestones?state=all&per_page=100"
        )
        ...
        if current_release_number and current_release_number[1] != "closed":
            ...
            self._gh_api_json(
                f"repos/apolloconfig/apollo-java/milestones/{number}",
                method="PATCH",
                fields={"state": "closed"},
            )
        ...
        if next_milestone_number is None:
            created = self._gh_api_json(
                "repos/apolloconfig/apollo-java/milestones",
                method="POST",
    ...
    def _write_changes_template(self, next_release: str, milestone_number: int) -> None:
        ...
        template = (
            ...
            "\n"
            "------------------\n"
            f"All issues and pull requests are [here](https://github.com/apolloconfig/apollo-java/milestone/{milestone_ref}?closed=1)\n"
        )
        if self.args.dry_run:
    ...
    def _promote_release_if_ready(self) -> None:
        if self._step_done("release_promoted"):
            return
        if not self._step_done("prerelease_created"):
            return
        if not self._step_done("release_workflow_completed"):
            return
    ...
    def _render_release_pr_body(release_version: str) -> str:
        return (
            ...
            f"Prepare Apollo Java {release_version} release by removing `-SNAPSHOT` from root revision.\n\n"
    ...
    def _render_post_release_pr_body(next_release: str) -> str:
        return (
            "## What's the purpose of this PR\n\n"
            f"Post-release housekeeping for Apollo Java {next_release}: bump next SNAPSHOT and archive CHANGES.md.\n\n"
            "## Which issue(s) this PR fixes:\n"
            "Fixes #N/A (release task)\n\n"
            "## Brief changelog\n\n"
    ...
    def _print_final_report(self) -> None:
        report = {
            ...
            "release_workflow_url": self.state.get("release_workflow_url"),
